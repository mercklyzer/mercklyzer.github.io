...
const Preview = ({title, image, preview, visitLink}) => {
    if(visitLink){
        return (<a href={visitLink}
            target="__blank"
            className="group cursor-pointer">
            <img className="object-contain group-hover:hidden" src={image} alt={`${title} preview`}/>
            <img className="object-contain hidden group-hover:block" src={preview} alt={`${title} preview`}/>
        </a>)
    }
...
const Visit = ({visitLink}) => {
    ...
    return (
        <span className="text-sm font-medium  opacity-60 block">Visit</span>
    )
}
...
const Project = ({image, title, visitLink, preview, documentationLink}) => {
    ...
    <div className="project flex flex-col justify-center items-center w-full opacity-0 ">
        <Preview title={title} image={image} preview={preview} visitLink={visitLink}/>
        <div className="flex justify-between items-center w-full  text-white mt-2 font-poppins">
            <div className="font-semibold text-md">{title}</div>
            <div>
                <Visit visitLink={visitLink}/>
